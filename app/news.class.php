...
class News{
    ...
    public function getNews($params){
        ...
        try{
            ...
            //$sql = "SELECT $s_fields FROM news AS n INNER JOIN tf_idf AS t ON n.url=t.url INNER JOIN bert AS b ON n.url=b.url $sql_where ORDER BY date DESC LIMIT $s_start, $s_limit";
            //wrlog($sql);
            $sth = DB::query($sql);
